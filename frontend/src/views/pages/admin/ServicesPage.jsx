...
import { mockServices, formatVND } from '../../../data/mockData';
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {mockServices.map(svc => (
          <div key={svc.MaDV} className="bg-surface-container-lowest rounded-2xl p-6 shadow-[0_2px_12px_rgba(77,70,53,0.06)] hover:shadow-[0_8px_32px_rgba(77,70,53,0.1)] transition-all duration-300 group">
            <div className="flex items-start justify-between mb-4">
              <div className="w-12 h-12 rounded-2xl bg-amber-50 flex items-center justify-center group-hover:bg-primary-container transition-colors"><span className="material-symbols-outlined text-2xl text-primary">{svc.Icon}</span></div>
              <StatusBadge status={svc.TrangThai} />
            </div>
            <h3 className="font-semibold text-on-surface font-notoSerif mb-2">{svc.TenDV}</h3>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-surface-container-high">
              <span className="text-primary font-bold text-sm">{svc.Gia > 0 ? `${formatVND(svc.Gia)}/${svc.DonVi}` : 'Miễn phí'}</span>
