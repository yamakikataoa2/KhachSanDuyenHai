...
export default function StaffPage() {
  ...
  useEffect(() => {
    ...
    adminService.getStaff()
      .then(res => {
        if (isMounted) setStaff(Array.isArray(res) ? res : res.data || []);
      })
  ...
  if (loading) {
    return <div className="space-y-6 animate-fade-in"><PageHeader title="Nhân viên" description="Quản lý đội ngũ nhân viên khách sạn" icon="badge" /><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6"><SkeletonCard /></div></div>;
  }
          ...
          <div key={nv.MaNhanVien} className="bg-surface-container-lowest rounded-2xl p-6 shadow-[0_2px_12px_rgba(77,70,53,0.06)] hover:shadow-[0_8px_32px_rgba(77,70,53,0.1)] transition-all duration-300">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-on-surface-variant mt-0.5">{nv.taiKhoan?.vaiTro?.TenVaiTro || '—'}</p>
              </div>
